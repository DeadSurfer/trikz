...
public Action StuckCmd(int client, any args)
{
	...
	if(GetConVarInt(c_Limit) > 0)
	{
		...
		{
			CPrintToChat(client, "{white}You must wait {orange}%i {white}seconds before use this feature again.", RoundFloat(g_fTime[client] - GetGameTime()));
	...
	{
		CPrintToChat(client, "{white}Unstuck is already in progress, {orange}%i {white}checks completed so far.", g_iStuckCheck[client]);
	...
	{
		CPrintToChat(client, "{white}Attempting unstuck in {orange}%i {white}seconds.", GetConVarInt(c_Delay_H));
...
{	
	...
	else
	{
		...
		{
			CPrintToChat(client, "{white}You do not appear to be stuck.");
		...
		{
			CPrintToChat(client, "{white}Your position has been fixed, you should now be unstuck.");
			//Enable controls
			int flags = GetEntityFlags(client) & ~FL_ATCONTROLS;
			SetEntityFlags(client, flags);
			g_iStuckCheck[client] = -1;
...
void TryFixPosition(int client, float Radius, float pos_Z, float DegreeAngle)
{
	...
	{
		CPrintToChat(client,"{white}Something went wrong, if you are still stuck try /stuck again or call an admin.");
		if(DelayTimer[client] != null)
		{
			KillTimer(DelayTimer[client]);
			DelayTimer[client] = null;
	...
	CPrintToChat(client,"{white}Unable to fix your position, please call for admin assistance.");
